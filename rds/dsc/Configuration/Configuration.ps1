...
configuration RDSDeployment
{
   param 
    ( 
        ...
        [String]$collectionDescription = "A Remote Desktop Services Collection."
    ...
    if (-not $externalfqdn)       { $externalfqdn = $webAccessServer }

    if ($sessionHostNamingPrefix)
    { 
        $sessionHosts = @( 0..($numberOfRdshInstances) | % { "$sessionHostNamingPrefix$_.$domainname"} )
